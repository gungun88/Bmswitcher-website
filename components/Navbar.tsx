    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-2 lg:gap-4">
            ...
            <span onClick={() => handleNavClick('#/features')} className={getNavLinkClass('#/features')}>{t('nav.features')}</span>
            <span onClick={() => handleNavClick('#/privacy')} className={getNavLinkClass('#/privacy')}>{t('nav.privacy')}</span>
            <span onClick={() => handleNavClick('#/about')} className={getNavLinkClass('#/about')}>{t('nav.about')}</span>
      ...
      {mobileMenuOpen && (
        <div className="absolute top-full left-0 right-0 bg-white shadow-lg border-t border-gray-100 md:hidden p-4 flex flex-col gap-4 animate-in slide-in-from-top-2">
          ...
          <span onClick={() => handleNavClick('#/features')} className={`block p-3 rounded-lg cursor-pointer ${isActive('#/features') ? 'bg-blue-50 text-arco-primary font-medium' : 'text-gray-600'}`}>{t('nav.features')}</span>
          <span onClick={() => handleNavClick('#/privacy')} className={`block p-3 rounded-lg cursor-pointer ${isActive('#/privacy') ? 'bg-blue-50 text-arco-primary font-medium' : 'text-gray-600'}`}>{t('nav.privacy')}</span>
          <span onClick={() => handleNavClick('#/about')} className={`block p-3 rounded-lg cursor-pointer ${isActive('#/about') ? 'bg-blue-50 text-arco-primary font-medium' : 'text-gray-600'}`}>{t('nav.about')}</span>
          <div className="pt-2">
